...
def match_otc_strategy(item: dict, direction: str, confidence: int) -> tuple[str, str]:
    ...
    if direction == "long":
        if f_score >= 25 and t_score >= 40:
            return (
                ...
                "采购方强偏多组合：累进宝4.0适合愿意承接下跌采购、换取区间补贴的场景；累进宝Plus适合希望保留下方补贴且参与上方突破；采省易3.0适合更重视上涨保护、同时接受下方1:1采购的客户。",
            )
        if f_score >= 25 and t_score < 40:
            return (
        ...
        if confidence >= 60:
            return (
                ...
                "技术偏多但基本面支撑一般：更适合低成本采购优化或有限保护，不建议裸多；采省易2.0/海鸥看涨更适合控制权利金和上方保护成本。",
            )
        return (
            "累进宝4.0 / 采省易3.0",
            "偏多但强度一般：累进宝4.0适合有采购计划且愿意承接下跌敞口的客户；采省易3.0适合更重视防涨保护和结构边界的客户；需确认真实采购计划，避免把套保结构做成投机多头。",
        )
    ...
    if f_score >= 30:
        return (
            ...
            "基本面偏多但综合信号未触发：采购方可先观察保护型结构，等价格站回关键均线后再考虑提高结构名义；气囊累计适合不想追多但担心上涨的防守采购。",
        )
    if f_score <= -30:
        return (
